...
class BaseAdmin(DjangoObjectActions, ModelAdmin):
    ...
    def get_list_filter(self, request):
        autocomplete_filters = [
            Autocomplete(_(f.title()), f) for f in self.autocomplete_list_filter
        ]
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        return (
            update + base if obj else create + base
        ) + self.global_readonly_fields

    def get_list_display(self, request):
        return self.list_display + [
            key
            for key in BaseModel.FIELDS
            if key not in self.excluded_list_display
        ]

    def get_fieldsets(self, request, obj=None):
        object_fieldsets = [
            self.make_fieldset_field(*fields, name=name)
            for fields, name in self.object_fieldsets
        ]
        meta_fieldsets = [
            self.make_fieldset_field(*fields, name=name)
            for fields, name in self.meta_fieldsets
        ]
    ...
    def save_model(self, request, obj, form, change):
        fields = (
            self.update_force_field_as_current_user
            if change
            else self.create_force_field_as_current_user
        )
