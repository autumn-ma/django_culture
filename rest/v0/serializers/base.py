...
class ConditionalSerializerMixin:
    ...
    def __init__(self, *a, **kw):
        self.exclude_relational_fields = kw.pop(
            "exclude_relational_fields", []
        )  # to remove bootstraps
        super().__init__(*a, **kw)

    def get_fields(self):
        fields = super().get_fields()  # NOQA
        relational_fields = getattr(
            self.Meta, self.CONDITIONAL_SERIALIZER_RELATIONAL_FIELDS_KEY, {}
        )  # NOQA
    ...
    def property_maker(cls, name, serializer):
        ...
        def generic_property(_self):
            ...
            include = (
                get and hasattr(get, "getlist") and get.getlist("include")
            ) or []
            name_in_get = name in list(
                chain.from_iterable([item.split(",") for item in include])
            )
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if hasattr(cls, "Meta"):  # NOQA
            if not hasattr(
                cls.Meta, cls.CONDITIONAL_SERIALIZER_RELATIONAL_FIELDS_KEY
            ):  # NOQA
                setattr(
                    cls.Meta,
                    cls.CONDITIONAL_SERIALIZER_RELATIONAL_FIELDS_KEY,
                    {},
                )  # NOQA
...
class OnlyCreateNoUpdateMixin:
    ...
    def get_extra_kwargs(self):
        ...
        create_no_update = getattr(
            self.Meta, self.ONLY_CREATE_NO_UPDATE_FIELDS_KEY, []
        )  # NOQA
...
class BaseModelSerializer(
    OnlyCreateNoUpdateMixin, ConditionalSerializerMixin, ModelSerializer
):
